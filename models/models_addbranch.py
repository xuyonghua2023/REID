...
class CrossBranchFusion(nn.Module):
    def __init__(self, in_channels, num_heads=4, dropout=0.1):
        super(CrossBranchFusion, self).__init__()
    ...
    def forward(self, branch_features):
        # branch_features: List of [batch_size, C, H, W]
        batch_size, C, H, W = branch_features[0].shape
        # 将特征图展平为序列，形状 [batch_size, num_branches, C]
        seq_features = [F.adaptive_avg_pool2d(f, (1, 1)).view(batch_size, -1) for f in branch_features]
        seq_features = torch.stack(seq_features, dim=1)  # [batch_size, num_branches, C]
        # Transformer 编码
        fused_seq = self.transformer(seq_features)  # [batch_size, num_branches, C]
        # 取平均特征
        fused_feature = fused_seq.mean(dim=1)  # [batch_size, C]
        # 重塑为特征图形式
        fused_ff = fused_feature.view(batch_size, C, 1, 1).expand(-1, -1, H, W)  # [batch_size, C, H, W]
        return fused_ff

# # 融合模块
# class CrossBranchFusion(nn.Module):
#     def __init__(self, in_channels, dim, num_heads=4, dropout=0.1):
#         super(CrossBranchFusion, self).__init__()
#         self.dim = dim
#         self.transformer = nn.TransformerEncoder(
#             nn.TransformerEncoderLayer(
...
#             num_layers=1
#         )
#         self.fc = nn.Linear(in_channels, dim)
#         self.norm = nn.LayerNorm(dim)
#         self.cls_head = nn.Linear(dim, 2048)  # 假设分类头输出维度为 2048，需根据数据集调整
...
#         fused_feature = fused_seq.mean(dim=1)  # [batch_size, C]
#         # 投影到目标维度
#         fused_emb = self.norm(self.fc(fused_feature))  # [batch_size, dim]
#         # 生成分类预测
#         fused_pred = self.cls_head(fused_emb)  # [batch_size, 2048]
#         # 重塑为特征图形式
#         fused_ff = fused_emb.view(batch_size, self.dim, 1, 1)  # [batch_size, dim, 1, 1]
#         return fused_ff, fused_emb, fused_pred
...
class base_branches(nn.Module):
    ...
    def __init__(self, backbone="ibn", stride=1):
        ...
        # 去掉后面的三层
        # print(len(list(model_ft.children())))
        # print(list(model_ft.children())[-3])
        # print(list(model_ft.layer4))
        # print(list(model_ft.children())[-2])
...
class FinalLayer(nn.Module):
    def __init__(self, class_num, n_branches, n_groups, losses="LBS", droprate=0, linear_num=False, return_f = True, circle_softmax=False, n_cams=0, n_views=0, LAI=False, x2g=False,x4g=False):
        ...
        if n_groups > 0:
            ...
            # 多一个融合分支
            for i in range(len(n_branches) + 1): 
                if losses == "LBS":
                    if i % 2 == 0:
...
class MBR_model(nn.Module):         
    def __init__(self, class_num, n_branches, n_groups, losses="LBS", backbone="ibn", droprate=0, linear_num=False, return_f = True, circle_softmax=False, pretrain_ongroups=True, end_bot_g=False, group_conv_mhsa=False, group_conv_mhsa_2=False, x2g=False, x4g=False, LAI=False, n_cams=0, n_views=0):
        ...
        # 融合块
        self.fusion = CrossBranchFusion(in_channels=2048, num_heads=4, dropout=0.1)
    ...
    def forward(self, x,cam, view):
        ...
        # 插入融合模块
        if len(output) > 1:  # 仅对多分支应用融合
            fused_ff = self.fusion(output)
            output.append(fused_ff)
